...
def parse_args():
    ...
    parser.add_argument("--saving-step", type=int, default=100, help="Save model steps")
...
IMG_MEAN = np.array((94.095, 102.306, 108.12), dtype=np.float32) # means of Target Data
def load_single_image(name):
    image = Image.open(osp.join(root_base + '/dataset/cityscapes/', "images/%s" % (name))).convert('RGB')
    # resize
...
class Base_Adaptation():
    def __init__(self,model_loader):
        ...
        self.cnt_img = 0
        self.entropy_th_class = np.ones((1,4)) #* self.args.entropy_th
    ...
    def class_wise_sort(self):
        ...
        flops, params = profile(self.model.to(device), inputs=(input,))
        params_in_M = params / 10 ** 6
        print("参数量111：", params_in_M, "M")
        # 将FLOPS转换为GFLOPS
        flops_in_GFLOPS = flops / 10 ** 9
        print("FLOPS：", flops_in_GFLOPS, "GFLOPs")



        targetloader = CreateTrgDataLoader(self.args)
        
        data_save_class = {classes[0]:[],classes[1]:[],classes[2]:[],classes[3]:[],"f_name_"+classes[0]:[],"f_name_"+classes[1]:[],"f_name_"+classes[2]:[],"f_name_"+classes[3]:[]}
        data_save_class_sorted = {classes[0]:[],classes[1]:[],classes[2]:[],classes[3]:[],"f_name_"+classes[0]:[],"f_name_"+classes[1]:[],"f_name_"+classes[2]:[],"f_name_"+classes[3]:[]}
        ...
        for c in range(len(classes) ):      
            ...
            if(self.args.source == 'synthia'):
                if(c !=9 and c !=14 and c !=16):
                    ...
                    select_imgs =  int(len(f_name_arrays) * (self.args.p/4))
        ...
        for index, batch in tqdm.tqdm(enumerate(targetloader)):
            ...
            for patch in range(self.args.no_of_patches_per_image):
                ...
                if(self.args.patch_size[1] == 256):
                    ...
                    y_PATCH =  Y_pred[0, point_Y:point_Y+point_Y_s, point_X:point_X+point_X_s].reshape(1,point_Y_s,point_X_s,4)
                    ...
                    y_PATCH = np.rollaxis(y_PATCH[0], axis = -1).reshape(1,4,point_Y_s,point_X_s)
                    ...
                    Y_Patch_resize = np.rollaxis(Y_Patch_resize, axis = -1).reshape(1,IMG_H,IMG_W,4)
                ...
                self.save_data(X_Patch_resize, Y_Patch_resize.reshape(IMG_H,IMG_W,4), self.args.generated_data,(count+patch),map_patch_re)
    ...
    def train_adp(self,round_):
        ...
        targetloader_iter, sourceloader_iter = enumerate(targetloader), enumerate(sourceloader)
        start_iter = 0
...
if __name__ == "__main__":
    ...
    if args.model == 'DeepLab':
        args.restore_from   = os.path.join(root_base, 'init_models', model_weights+'.pth')
    else:
    ...
    model_initl.args.total_no_of_target = 429         # Total number of target images
    model_initl.args.patch_size = (256,256)
