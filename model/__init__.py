...
def CreateModel(args):
    if args.model == 'DeepLab':
        model = Deeplab(num_classes=args.num_classes, init_weights=args.restore_from, restore_from=None, phase=args.set)
        if args.set == 'train':
    ...
    if args.model == 'VGG':
        model = VGG16_FCN8s(num_classes=4, init_weights=args.init_weights, restore_from=args.restore_from)
        if args.set == 'train':
